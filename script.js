...
const determineWinner = async () => {
    ...
    let text = `
Tu mano es ${playerHand} con un valor de ${playerValue}.
La mano del distribuidor es ${dealerHand} con un valor de ${dealerValue}.
${playerValue > dealerValue ? "<em>¡Tú ganas!</em>" : "<em>¡Dealer gana!</em>"}
    `
    showNotice(text)
}
...
const hitDealer = async () => {
    ...
    if (handValue <= 16) {
        ...
        showNotice("¡El distribuidor tiene 21! ¡El dealer gana!")
    }
    else if (handValue > 21) {
        showNotice("¡Busto del distribuidor! ¡Tú ganas!")
    }
    else {
        determineWinner()
    }
...
const hitPlayer = async () => {
    ...
    if (handValue <= 21) {
        ...
        let text = `Busto! Su mano es ${playerHand} con un valor de ${handValue}.`
        showNotice(text)
    }
}
